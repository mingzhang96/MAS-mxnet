...
class MAS_Omega_Update(mx.optimizer.Optimizer):
    ...
    **lazy updates** are applied by::
    ...
    """
    def __init__(self, momentum=0.0, lazy_update=True, reg_params={}, batch_size=1, **kwargs):
        super(MAS_Omega_Update, self).__init__(**kwargs)
    ...
    def _update_impl(self, index, weight, grad, state, multi_precision=False):
        ...
        if index in self.reg_params:
            ...
            grad_copy = grad.copy()
            prev_size = self.batch_index * self.batch_size
            curr_size = (self.batch_index + 1) * self.batch_size
            omega = reg_param['omega']
            omega = omega * prev_size
            omega = omega + mx.ndarray.abs(grad_copy)
            omega = omega / curr_size
            reg_param['omega'] = omega
            self.reg_params[index] = reg_param
        self.batch_index += 1
        # ----------- MAS end -----------
